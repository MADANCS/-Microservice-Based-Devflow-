...
foreach ($svc in $services) {
    ...
    Start-Process -FilePath $JAVA_EXE -ArgumentList "-jar", "`"$jarPath`"", "--server.port=$($svc.port)", "$excludeArg" -RedirectStandardOutput "$ROOT\logs\$($svc.name).log" -RedirectStandardError "$ROOT\logs\$($svc.name).err" -WindowStyle Hidden
...
if (Test-Path $frontendPath) {
    ...
    Start-Process powershell -ArgumentList "-WindowStyle", "Hidden", "-Command", "cd '$frontendPath'; npm run dev > '$ROOT\logs\frontend.log' 2>&1"
}
